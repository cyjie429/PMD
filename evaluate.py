...
from predict import PredictorModel
...
def compute_metrics(hr_tensor: torch.tensor,
    ...
    for start in tqdm.tqdm(range(0, total, batch_size)):
        ...
        # num = hr_tensor[start:end, :].shape[0]
        # stacked_hrt = torch.cat((hr_tensor[start:end, :], entities_tensor), dim=0)
        # batch_score = torch.corrcoef(stacked_hrt)[:num,num:]
        # del stacked_hrt
...
def predict_by_split():
    ...
    entity_tensor = predict_by_entities(predictor.model, entity_dict.entity_exs)

    forward_metrics = eval_single_direction(predictor.model,
                                            entity_tensor=entity_tensor,
                                            eval_forward=True)
    backward_metrics = eval_single_direction(predictor.model,
                                             entity_tensor=entity_tensor,
                                             eval_forward=False)
...
def eval_single_direction(model,
    ...
    hr_tensor, _ = predict_by_examples(model, examples)
...
@torch.no_grad()
def predict_by_entities(model, entity_exs, use_cuda=True) -> torch.tensor:
    examples = []
    for entity_ex in entity_exs:
        examples.append(Example(head_id='', relation='',
                                tail_id=entity_ex.entity_id))
    data_loader = torch.utils.data.DataLoader(
        Dataset(path='', examples=examples, task=args.task, train_flag=False),
        num_workers=2,
...
@torch.no_grad()
def predict_by_examples(model, examples: List[Example], use_cuda=True):
    data_loader = torch.utils.data.DataLoader(
        Dataset(path='', examples=examples, task=args.task, train_flag=False),
        num_workers=1,
