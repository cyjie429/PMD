...
class Example:
    ...
    def vectorize(self, train_flag=True) -> dict:
        ...
        if train_flag:
            hr_token_mask = torch.randint(low=1, high=len(hr_encoded_inputs['input_ids'])-1, size=(int(0.0 * len(hr_encoded_inputs['input_ids'])), ))
            tail_token_mask = torch.randint(low=1, high=len(tail_encoded_inputs['input_ids'])-1, size=(int(0.0 * len(tail_encoded_inputs['input_ids'])), ))
            head_token_mask = torch.randint(low=1, high=len(head_encoded_inputs['input_ids'])-1, size=(int(0.0* len(head_encoded_inputs['input_ids'])), ))
        else:
            hr_token_mask = torch.randint(0, 1, size=(0, ))
            tail_token_mask = torch.randint(0, 1, size=(0, ))
            head_token_mask = torch.randint(0, 1, size=(0, ))

        student_hr_token_ids = hr_encoded_inputs['input_ids']
        student_tail_token_ids = tail_encoded_inputs['input_ids']
        student_head_token_ids = head_encoded_inputs['input_ids']

        if hr_token_mask.numel() != 0:
            for i in hr_token_mask:
                hr_encoded_inputs['input_ids'][i] = 103
        if tail_token_mask.numel() != 0:        
            for i in tail_token_mask:
                tail_encoded_inputs['input_ids'][i] = 103 
        if head_token_mask.numel() != 0:   
            for i in head_token_mask:
                head_encoded_inputs['input_ids'][i] = 103        
        ...
        return {'hr_token_ids': hr_encoded_inputs['input_ids'],
                ...
                'hr_token_mask': hr_token_mask,
                'tail_token_mask': tail_token_mask,
                'head_token_mask': head_token_mask,
                'student_hr_token_ids': student_hr_token_ids, 
                'student_tail_token_ids': student_tail_token_ids, 
                'student_head_token_ids': student_head_token_ids, 
                'obj': self}


class Dataset(torch.utils.data.dataset.Dataset):

    def __init__(self, path, task, train_flag=True, examples=None):
    ...
    def __getitem__(self, index):
        return self.examples[index].vectorize(train_flag=self.train_flag)
...
def collate(batch_data: List[dict]) -> dict:
    ...
    student_hr_token_ids, _ = to_indices_and_mask(
        [torch.LongTensor(ex['student_hr_token_ids']) for ex in batch_data],
        pad_token_id=get_tokenizer().pad_token_id)
    ...
    tail_token_ids, tail_mask = to_indices_and_mask(
        ...
        pad_token_id=get_tokenizer().pad_token_id)
    student_tail_token_ids, _ = to_indices_and_mask(
        [torch.LongTensor(ex['student_tail_token_ids']) for ex in batch_data],
        pad_token_id=get_tokenizer().pad_token_id)
    ...
    head_token_ids, head_mask = to_indices_and_mask(
        ...
        pad_token_id=get_tokenizer().pad_token_id)
    student_head_token_ids, _ = to_indices_and_mask(
        [torch.LongTensor(ex['student_head_token_ids']) for ex in batch_data],
        pad_token_id=get_tokenizer().pad_token_id)
    ...
    hr_token_mask = [ex['hr_token_mask'] for ex in batch_data]
    hr_token_mask_tensor = torch.Tensor(torch.nn.utils.rnn.pad_sequence([torch.as_tensor(s) for s in hr_token_mask], batch_first=True, padding_value=-1))
    tail_token_mask = [ex['tail_token_mask'] for ex in batch_data]
    tail_token_mask_tensor = torch.Tensor(torch.nn.utils.rnn.pad_sequence([torch.as_tensor(s) for s in tail_token_mask], batch_first=True, padding_value=-1))
    head_token_mask = [ex['head_token_mask'] for ex in batch_data]
    head_token_mask_tensor = torch.Tensor(torch.nn.utils.rnn.pad_sequence([torch.as_tensor(s) for s in head_token_mask], batch_first=True, padding_value=-1))
    ...
    batch_dict = {
        ...
        'hr_token_mask': hr_token_mask_tensor,
        'tail_token_mask': tail_token_mask_tensor,
        'head_token_mask': head_token_mask_tensor,
        'student_hr_token_ids': student_hr_token_ids, 
        'student_tail_token_ids': student_tail_token_ids, 
        'student_head_token_ids': student_head_token_ids, 
