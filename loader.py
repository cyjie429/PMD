...
class TeacherModelLoader:
    ...
    def load(self, ckt_path, use_data_parallel=False):
        ...
        self.teacher_args.__dict__ = ckt_dict['args']
        self._setup_args()
        build_tokenizer(self.teacher_args)
        self.model = build_model(self.teacher_args)
    ...
    def _setup_args(self):
        for k, v in args.__dict__.items():
            if k not in self.teacher_args.__dict__:
                logger.info('Set default attribute: {}={}'.format(k, v))
                self.teacher_args.__dict__[k] = v

        self.teacher_args.__dict__["teacher_bert_num_hidden_layers"] = args.teacher_bert_num_hidden_layers
        logger.info('Args used in training: {}'.format(json.dumps(self.teacher_args.__dict__, ensure_ascii=False, indent=4)))
        args.use_link_graph = self.teacher_args.use_link_graph
        args.is_test = False
